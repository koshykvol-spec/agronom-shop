...
async function tryGemini(apiKey, sys, prompt, image_b64, image_type) {
  ...
  try {
    ...
    return { error: 'Gemini: ' + String(e.message || e) };
  }
  clearTimeout(timeoutId);
...
async function tryOpenRouter(apiKey, sys, prompt, image_b64, image_type) {
  ...
  try {
    ...
    clearTimeout(timeoutId);
    return { error: 'OpenRouter: ' + String(e.message || e) };
  }
  clearTimeout(timeoutId);
...
// Пускає кілька ключів ОДНОГО провайдера ПАРАЛЕЛЬНО — виграє перша успішна відповідь.
// Це обмежує час очікування ~9с замість підсумовування часу по кожному ключу окремо.
async function raceKeys(keys, tryFn, sys, prompt, image_b64, image_type) {
  if (keys.length === 0) return { text: '', error: 'пул ключів порожній' };
  const attempts = keys.map(k =>
    tryFn(k, sys, prompt, image_b64, image_type).then(r => r.text ? r : Promise.reject(r.error || 'помилка'))
  );
  try {
    return await Promise.any(attempts);
  } catch (e) {
    const msgs = (e && e.errors) ? e.errors.map(String) : [String(e)];
    return { text: '', error: msgs[msgs.length - 1] || 'усі ключі невдалі' };
  }
}
...
export async function onRequestPost(context) {
  ...
  try {
    ...
    // 1) Gemini — основний пул: до 3 ключів по черзі (ротація), пробуємо їх ПАРАЛЕЛЬНО
    const geminiKeys = (await getRotatedKeys(env.DB, 'gemini_api_key')).slice(0, 3);
    let { text: rawText, error: lastErr } = await raceKeys(geminiKeys, tryGemini, sys, prompt, image_b64, image_type);
    ...
    if (!rawText) {
      ...
      const r2 = await raceKeys(orKeys, tryOpenRouter, sys, prompt, image_b64, image_type);
      rawText = r2.text;
