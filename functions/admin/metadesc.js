// /admin/metadesc — масова заливка SEO-описів (meta description) товарів.
// Вхід: CSV/TSV (sku/назва ; meta description)
// АБО JSON [{"sku":"...","meta_desc":"..."}]. Авто-визначення формату.
// Матч по sku (точно), відкат на точну назву (name або display_name). Dry-run → залити.
// Чіпає ЛИШЕ product_content.meta_desc; ціни/наявність/фото/анотація — не торкаємось.
...
function parseRecords(text) {
  ...
  if (t[0] === '[' || t[0] === '{') {
    ...
    return arr.map(function(r) {
      ...
      var val = r.meta_desc || r.meta_description || r.description || r.desc || r['seo description'] || r['seo_desc'] || r.annotation || r.text || '';
      return { id: id, meta_desc: String(val).trim() };
    });
  }
  ...
  const isHeader = ['sku', 'артикул', 'код', 'id', 'назва', 'name'].includes(h0)
    || ['meta_desc', 'meta description', 'seo description', 'description', 'опис', 'annotation', 'текст'].includes(h1);
  return rows.slice(isHeader ? 1 : 0)
    .map(r => ({ id: (r[0] || '').trim(), meta_desc: r.slice(1).join(delim) }));
}
...
async function exportEmptyJson(db, url) {
  ...
  const rows = (await db.prepare(
    `SELECT p.sku, COALESCE(NULLIF(c.display_name,''), p.name) AS name, p.category AS category, p.brand AS brand,
            COALESCE(c.active_ingredient,'') AS ai, COALESCE(c.annotation,'') AS annotation
  ...
  const items = rows.map(r => {
    ...
    if (r.annotation) o.annotation = r.annotation.slice(0, 300);
    return o;
  });
  return new Response(JSON.stringify(items, null, 2), {
    headers: {
      'content-type': 'application/json; charset=utf-8',
      'content-disposition': 'attachment; filename="tovary-bez-metadesc.json"',
...
<title>SEO description (масово)</title><style>
...
<h2>🔍 Масове написання SEO description</h2>
<p class=muted>Заповнює мета-описи товарів пакетом. Матч по <b>SKU</b> (точно), якщо не знайдено — по точній назві.
Чіпає лише meta_desc; анотація/ціни/наявність/фото не змінюються.</p>
<p class=muted>Формат — на вибір (визначається автоматично):</p>
<ul class=muted style="margin-top:0">
  <li><b>CSV/TSV</b> з Excel/Таблиць: 1-й стовпець — SKU (або назва), 2-й — SEO description. Роздільник <code>,</code> <code>;</code> або таб.</li>
  <li><b>JSON</b>: <code>[{"sku":"00-123","meta_desc":"текст…"}]</code></li>
</ul>
<div style="background:#fff;border:1px solid #e0e8e0;border-radius:10px;padding:12px;margin:12px 0">
  <b>Крок 1 · Експорт товарів без SEO description</b><br>
  <span class=muted>Віддає JSON-список товарів (з анотацією як джерелом) — готовий вхід для LLM. Поле «annotation» включено щоб LLM міг скоротити його до 160 символів.</span><br>
  <label class=muted style="display:inline-block;margin:8px 8px 0 0">які:
    <select id=exmode style="padding:5px;border:1px solid #c8e0c8;border-radius:6px">
      <option value=empty>без meta_desc</option>
      <option value=short>короткі «стуби» (&lt; N)</option>
      <option value=weak>порожні + короткі</option>
    </select></label>
  <label class=muted style="display:inline-block;margin:8px 8px 0 0">N&nbsp;симв: <input type=number id=exmax value=160 min=20 max=5000 style="width:78px;padding:5px;border:1px solid #c8e0c8;border-radius:6px"></label>
...
<details style="margin:10px 0;border:1px solid #e0e8e0;border-radius:10px;background:#fff;padding:8px 12px">
  ...
  <textarea id=prompt readonly style="min-height:260px;margin-top:8px;background:#fafafa">Ти — SEO-копірайтер інтернет-магазину агротоварів «Агроном» (м. Володимир, Україна).
Напиши SEO description (meta description) для кожного товару зі списку нижче.

ФОРМАТ ВІДПОВІДІ — лише валідний JSON-масив, без markdown і пояснень.
Кожен елемент: {"sku":"…точно як у вході…","meta_desc":"…твій опис…"}

ПРАВИЛА:
- Мова: українська. Лише звичайний текст — БЕЗ HTML, markdown, лапок на початку/кінці.
- Довжина: 130–160 символів (суворо, Google обрізає довше).
- Зміст: коротко що це, ключова користь, заклик до дії («Купити в магазині Агроном»).
- Якщо є поле annotation — скороти його до суті + додай заклик. Якщо нема — склади з name, category, brand.
- НЕ вигадуй властивостей яких немає в даних. Без «води» і порожніх обіцянок.
- sku не змінюй і не вигадуй. Один товар = один об'єкт у масиві.

СПИСОК ТОВАРІВ (JSON) — встав нижче вміст файлу tovary-bez-metadesc.json:
</textarea>
</details>
...
<p><textarea id=ta placeholder='00-12345,Агрітокс — гербіцид на основі МЦПА для зернових культур. Знищує дводольні бур&#39;яни. Купити в Агронома.'></textarea></p>
<div style="margin:6px 0"><b>Якщо meta_desc уже є в базі:</b>
  <select id=cpolicy style="padding:5px;border:1px solid #c8e0c8;border-radius:6px">
    <option value=weak>перезаписати лише короткі (&lt; N) — довгі лишити</option>
    <option value=all>перезаписати всі наявні</option>
    <option value=empty>не чіпати наявні (залити лише порожні)</option>
  </select>
  <span class=muted>N&nbsp;симв: <input type=number id=cmax value=120 min=20 max=5000 style="width:64px;padding:4px;border:1px solid #c8e0c8;border-radius:6px"></span>
  <div class=muted>У перегляді (Крок 4) кожен наявний запис видно «було→стане»; галочкою «🔒 лишити» можна вберегти конкретний.</div>
</div>
...
async function post(dry){
  ...
  var cm=$('cmax')?($('cmax').value||'120'):'120';
  var ex=dry?'':collectKeep().join(',');
  const u='/admin/metadesc?'+(dry?'dryrun=1&':'')+'policy='+pol+'&cmax='+encodeURIComponent(cm)+(ex?'&exclude='+encodeURIComponent(ex):'');
...
function render(d,dry){
  ...
  h+=sect('✅ Оновлення (поточний meta_desc → новий)',d.matched,function(x){
    var badge = !x.had ? '<span style="color:#2d6a2d">новий</span>'
      : (x.weak ? '<span style="color:#c0392b">🔁 перезапис короткого ('+x.curLen+' симв)</span>'
                : '<span style="color:#b8860b">перезапис наявного ('+x.curLen+' симв)</span>');
    ...
    return '<div style="margin-bottom:7px;border-bottom:1px solid #f2f2f2;padding-bottom:5px"><b>'+esc(x.sku)+'</b> · '+esc(x.n)+' — '+badge+keep+was
      +'<div>стане ('+x.newLen+' симв): '+esc(x.preview)+'</div></div>';
  });
  h+=sect('❓ Не знайдено',d.unmatchedList,x=>'<div>'+esc(x)+'</div>');
  h+=sect('⚠️ Неоднозначні',d.ambiguous,x=>'<div><b>'+esc(x.id)+'</b>: '+esc((x.names||[]).join('  |  '))+'</div>');
  h+=sect('🔒 Лишено наявні',d.skippedList,x=>'<div><b>'+esc(x.sku)+'</b> · '+esc(x.n)+' <span class=muted>('+(x.why||'')+', '+(x.curLen||0)+' симв)</span></div>');
  return h;
}
...
async function exportEmpty(){
  ...
  var mx=Math.max(20,Math.min(5000,parseInt($('exmax').value||'160',10)));
  var cat=$('excat')?$('excat').value:'', sub=$('exsub')?$('exsub').value:'';
  $('exs').textContent='готую…';
  var r=await fetch('/admin/metadesc?export=1&limit='+n+'&mode='+mode+'&maxlen='+mx
  ...
  var a=document.createElement('a'); a.href=URL.createObjectURL(blob); a.download='tovary-bez-metadesc.json';
  document.body.appendChild(a); a.click(); a.remove(); URL.revokeObjectURL(a.href);
  $('exs').textContent='⬇️ завантажено '+cnt+' товарів (усього без meta_desc: '+remaining+'). Згенеруй у LLM → встав у Крок 3 → «Перевірити».';
}
function copyPrompt(){
...
export async function onRequestPost(context) {
  ...
  let cmax = parseInt(u.searchParams.get('cmax') || '120', 10);
  if (!(cmax > 0)) cmax = 120;
  ...
  const ex = (await db.prepare(
    `SELECT p.pid, p.sku, p.name, COALESCE(c.display_name,'') dn,
            CASE WHEN c.pid IS NULL THEN NULL ELSE COALESCE(c.meta_desc,'') END md
  ...
  const Up = db.prepare(`INSERT INTO product_content(pid, meta_desc) VALUES(?, ?)
                         ON CONFLICT(pid) DO UPDATE SET meta_desc=excluded.meta_desc`);
  ...
  for (const rec of recs) {
    ...
    const md = String(rec.meta_desc == null ? '' : rec.meta_desc).trim().slice(0, MAXLEN);
    ...
    const had = cur.md != null && cur.md !== '';
    const curLen = (cur.md || '').length;

    if (had) {
      const manualKeep = exclude.has(String(cur.sku || ''));
      const policyKeep = policy === 'empty' ? true
                       : policy === 'weak'  ? (curLen >= cmax)
                       : false;
    ...
    if (matched.length < 80) matched.push({
      ...
      curLen, weak: had && curLen < cmax,
      curPrev: had ? (cur.md || '').slice(0, 70) : '',
      preview: md.slice(0, 90) + (md.length > 90 ? '…' : ''),
      newLen: md.length
    });
    if (!dry) stmts.push(Up.bind(pid, md));
  }
