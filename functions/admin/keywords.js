...
// Текст → масив {id, keywords}. Підтримує JSON-масив і CSV/TSV.
function parseRecords(text) {
  ...
  if (t[0] === '[' || t[0] === '{') {
    ...
    return arr.map(function(r) {
      ...
      var val = r.keywords || r.keyword || r.kw || r['ключові слова'] || r['ключові'] || r.annotation || r.anno || r.text || '';
      return { id: id, keywords: String(val).trim() };
    });
  }
...
// Експорт товарів-кандидатів на (пере)заповнення у JSON — вхід для LLM.
// ?export=1&limit=N&mode=empty|short|weak&maxlen=250
async function exportEmptyJson(db, url) {
  let limit = parseInt(url.searchParams.get('limit') || '50', 10);
  if (!(limit > 0)) limit = 50;
  limit = Math.min(limit, 500);
  let maxlen = parseInt(url.searchParams.get('maxlen') || '100', 10);
  ...
  return new Response(JSON.stringify(items, null, 2), {
    headers: {
      ...
      'content-disposition': 'attachment; filename="tovary-bez-klyuchsliv.json"',
...
<title>Ключові слова (масово)</title><style>
...
<h2>🔑 Масове написання ключових слів</h2>
<p class=muted>Заповнює ключові слова товарів пакетом. Матч по <b>SKU</b> (точно), якщо не знайдено — по точній назві.
Чіпає лише keywords; ціни/наявність/фото не змінюються.</p>
<p class=muted>Формат — на вибір (визначається автоматично):</p>
<ul class=muted style="margin-top:0">
  <li><b>CSV/TSV</b> з Excel/Таблиць: 1-й стовпець — SKU (або назва), 2-й — ключові слова. Роздільник <code>,</code> <code>;</code> або таб.</li>
  <li><b>JSON</b>: <code>[{"sku":"00-123","keywords":"гербіцид кукурудза бур'яни…"}]</code></li>
</ul>
<div style="background:#fff;border:1px solid #e0e8e0;border-radius:10px;padding:12px;margin:12px 0">
  ...
  <span class=muted>Віддає JSON-список товарів — готовий вхід для LLM. Наступний експорт дає наступну порцію (оновлені зникають). «Короткі стуби» — ключові слова коротші за N символів.</span><br>
  ...
  <label class=muted style="display:inline-block;margin:8px 8px 0 0">N&nbsp;симв: <input type=number id=exmax value=100 min=10 max=5000 style="width:78px;padding:5px;border:1px solid #c8e0c8;border-radius:6px"></label>
...
<details style="margin:10px 0;border:1px solid #e0e8e0;border-radius:10px;background:#fff;padding:8px 12px">
  ...
  <textarea id=prompt readonly style="min-height:280px;margin-top:8px;background:#fafafa">Ти — SEO-спеціаліст інтернет-магазину агротоварів «Агроном» (м. Володимир, Україна).
Напиши ключові слова для кожного товару зі списку нижче.
...
ПРАВИЛА:
- Мова: українська. Слова/фрази через пробіл або кому.
- Обсяг: 8–15 слів або коротких фраз на товар.
- Включай: назву препарату (і скорочення/синоніми), тип (гербіцид/фунгіцид/інсектицид/добриво), культуру застосування, проблему (хвороба/шкідник/бур'ян), діючу речовину якщо є, бренд.
- НЕ повторюй слова. НЕ вигадуй властивостей. Спирайся лише на надані поля (name, category, brand, active_ingredient).
- Для агрохімікатів: обов'язково вкажи культуру та тип проблеми. Для добрив: культуру та елемент живлення.
- sku не змінюй і не вигадуй. Один товар = один об'єкт у масиві.

СПИСОК ТОВАРІВ (JSON) — встав нижче вміст файлу tovary-bez-klyuchsliv.json:
</textarea>
</details>
...
<p><textarea id=ta placeholder='00-12345,гербіцид кукурудза бур&apos;яни мишій лобода&#10;00-22222,фунгіцид яблуня парша борошниста роса'></textarea></p>
<div style="margin:6px 0"><b>Якщо ключові слова вже є в базі:</b>
  ...
  <span class=muted>N&nbsp;симв: <input type=number id=cmax value=50 min=5 max=5000 style="width:64px;padding:4px;border:1px solid #c8e0c8;border-radius:6px"></span>
...
function render(d,dry){
  ...
  h+=sect('✅ Оновлення (поточні ключові слова → нові)',d.matched,function(x){
...
async function exportEmpty(){
  ...
  var mx=Math.max(10,Math.min(5000,parseInt($('exmax').value||'100',10)));
  var cat=$('excat')?$('excat').value:'', sub=$('exsub')?$('exsub').value:'';
  $('exs').textContent='готую…';
  var r=await fetch('/admin/keywords?export=1&limit='+n+'&mode='+mode+'&maxlen='+mx
...
export async function onRequestPost(context) {
  ...
  let cmax = parseInt(u.searchParams.get('cmax') || '50', 10);
  if (!(cmax > 0)) cmax = 50;
  ...
  const ex = (await db.prepare(
    `SELECT p.pid, p.sku, p.name, COALESCE(c.display_name,'') dn,
            CASE WHEN c.pid IS NULL THEN NULL ELSE COALESCE(c.keywords,'') END kw
  ...
  const Up = db.prepare(`INSERT INTO product_content(pid, keywords) VALUES(?, ?)
                         ON CONFLICT(pid) DO UPDATE SET keywords=excluded.keywords`);
  ...
  for (const rec of recs) {
    ...
    const kw = String(rec.keywords == null ? '' : rec.keywords).trim().slice(0, MAXLEN);
    ...
    const had = cur.kw != null && cur.kw !== '';
    const curLen = (cur.kw || '').length;
    ...
    if (matched.length < 80) matched.push({
      ...
      curPrev: had ? (cur.kw || '').replace(/\s+/g, ' ').slice(0, 70) : '',
      preview: kw.replace(/\s+/g, ' ').slice(0, 90) + (kw.length > 90 ? '…' : ''),
      newLen: kw.length
    });
    if (!dry) stmts.push(Up.bind(pid, kw));
  }
